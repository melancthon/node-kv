...
Db.prototype.del = function (key, txn, val) {
    return this._db.del(this._keySanitize(key), this._valSanitize(val), txn && txn._txn);
};

Db.prototype.delDup = function (key, val, txn) {
    return this._db.del(this._keySanitize(key), this._valSanitize(val), txn && txn._txn);
};
